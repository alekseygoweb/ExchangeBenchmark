...
| `awg-params.sh` | Общая генерация параметров AWG 2.0: `H1–H4` диапазонами + CPS-пакет `I1` (DNS). |
...
| `update-obfuscation.sh` | Обновляет параметры обфускации сразу на сервере и во всех клиентах. |
...
### Как выглядят «правильные» параметры AWG 2.0 и как их обновить

Генератор (`awg-params.sh`) делает параметры в «официальном» виде:
- **`H1–H4` — диапазоны** `A-B` (тип заголовка «плавает» в диапазоне — сложнее
  фингерпринтить), непересекающиеся и `≤ 2147483647` (INT32_MAX).
- **`I1` — крафтовый CPS-пакет**, который парсится как настоящий **DNS-ответ**
  (`<r 2>` = случайный transaction id, далее тело ответа `A`-записи). Первый
  пакет соединения выглядит как обычный DNS, а не как высокоэнтропийный шум.

> Одиночные `H` и `I1 = <r 128>` (128 случайных байт) — тоже **валидный** AWG 2.0
> (его использует, например, установщик `bivlked/amneziawg-installer`), просто
> обфускация слабее против продвинутого DPI.

**Обновить уже работающий сервер + всех выданных клиентов** на новый формат
(с бэкапом; параметры меняются согласованно везде):
```bash
sudo bash update-obfuscation.sh          # H-диапазоны + CPS I1, спросит подтверждение
# sudo bash update-obfuscation.sh --all  # + перегенерировать Jc/Jmin/Jmax/S1–S4
# sudo bash update-obfuscation.sh --from /path/official.conf  # взять набор из офиц. конфига
```
После — **заново импортировать каждый клиентский `.conf`** (файлы обновлены в
`/root/awg-clients/`), т.к. параметры должны совпасть с сервером.
